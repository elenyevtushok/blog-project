...
export const PostItem = ({ post }: { post: Post }) => {
	...
	return (
		<div className="post-in">
			...
			<p className='post-author' data-testid={`post-author-${post.id}`}>Author: {user?.name}</p>
			<p>{post.body}</p>
			<div>
				...
				<Link to={`posts/${post.id}`}><button className="edit-post-button"><CommentOutlined /></button>
					</Link>
